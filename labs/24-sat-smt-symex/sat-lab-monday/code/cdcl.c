...
typedef struct {
    ...
    int    learnt;    // is it a learnt clause?
    int    watch0;    // index of the first watched literal in .lits
    int    watch1;    // index of the second watched literal in .lits
} clause_t;

// For each variable: the assigned value, the level it was assigned, and the reason (clause) if implied
typedef struct {
    val_t       value;
    int         level;
...
/* We'll keep watchers in an array of "watchers[list]" for each literal index */
typedef struct {
    clause_t **data;
    int        cap;
    int        size;
...
/* We'll keep a "trail" array of variables in order of assignment,
   plus an array "trail_lim" for the index at each decision level */
...
/* We'll fix the previously undeclared RESTART_INTERVAL here. */
static int    RESTART_INTERVAL = 50; // start smaller to force more random restarts
...
static inline int sign_of(int lit){
    return (lit>0)? 0 : 1;  // 0=pos,1=neg
}
// We'll map literal -> index: 
//   index( v>0 ) = 2*v
//   index( v<0 ) = 2*|v| + 1
...
static void cancel_until(int level){
    ...
    if(propQ> trail_sz) propQ= trail_sz; // ensure no out-of-bounds
}
...
static clause_t* propagate(void){
    while(propQ< trail_sz){
        ...
        int idx= lit_index(-lit); // watchers of the opposite
        watchlist_t *wl= &WATCHES[idx];
        int j=0;
...
static int pickPolarityRandom(int var){
    // 50% chance we pick TRUE, else FALSE
    int r= rand()%2;
    return (r? var : -var);
}
    ...
    while(1){
        ...
        if(confl){
            ...
            if(!enqueue( c->lits[0], c)){
                return 0; // conflict at level0
            }
        } else {
            // no conflict => check all assigned?
            int assigned=1;
            for(unsigned i=1; i<=N_VARS; i++){
                if(VARINFO[i].value==VAL_UNASSIGNED){
                    assigned=0;break;
            ...
            int var= decideVar();
            if(!var) return 1; // means all assigned
